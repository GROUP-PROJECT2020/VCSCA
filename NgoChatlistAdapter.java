...
public class NgoChatlistAdapter extends RecyclerView.Adapter<NgoChatlistAdapter.MyHolder> {

    Context context;
    List<NgoUserModel> userList;
    private HashMap<String, String> lastMessageMap;

    public NgoChatlistAdapter(Context context, List<NgoUserModel> userList) {
    ...
    public void onBindViewHolder(@NonNull MyHolder holder, int i) {
        ...
        try {
            Picasso.get().load(userImage).placeholder(R.drawable.person9).into(holder.profileIv);
        } catch (Exception e) {
            Toast.makeText(context, ""+e.getMessage(), Toast.LENGTH_LONG).show();
            Picasso.get().load(R.drawable.person9).into(holder.profileIv);
        }
